...
import { EntityArrayResponseType, UtenteService } from '../service/utente.service';
...
export class UtenteComponent implements OnInit {
  ...
  trackId = (_index: number, item: IUtente): number => this.utenteService.getUtenteIdentifier(item);

  search(query: string): void {
    if (query && UtenteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (UtenteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.utentes = this.refineData(dataFromBody);
  }

  protected refineData(data: IUtente[]): IUtente[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IUtente[] | null): IUtente[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.utenteService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.utenteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
